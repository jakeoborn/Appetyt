...
const CITY = 'phx';
const VAR = 'PHX_DATA';
const STATE_BOX_AZ = [31.3, 37.0, -114.9, -109.0];
...
function addrCity(a) { if (!a) return null; const m = a.match(/,\s*([A-Za-z][A-Za-z\s.'-]+?),\s*AZ\b/); return m ? m[1].trim() : null; }
...
for (const r of data) {
  ...
  checked++;
  if (!inBox(r.lat, r.lng, STATE_BOX_AZ)) continue;
...
fs.writeFileSync(path.join(__dirname, 'phx-neighborhood-audit.json'),
  JSON.stringify({ city: 'Phoenix', checked, in_bounds: inB, no_bounds: noB, severe: bySev.severe, moderate: bySev.moderate, edge: bySev.edge }, null, 2));
console.log('Phoenix: checked=' + checked + ' in=' + inB + ' no-bounds=' + noB + ' severe=' + bySev.severe.length + ' moderate=' + bySev.moderate.length + ' edge=' + bySev.edge.length);
