// Audit Houston cards whose (lat,lng) falls outside their claimed
// neighborhood's bounding box. Same format as other per-city audits.
...
function run() {
  ...
  fs.writeFileSync(
    path.join(__dirname, 'houston-neighborhood-audit.json'),
    JSON.stringify({ city: 'Houston', total_checked: checked, in_bounds: inBounds,
      out_of_bounds: outliers.length, no_bounds: noBounds, out_of_state: outOfState,
      severe: bySev.severe, moderate: bySev.moderate, edge: bySev.edge }, null, 2)
  );

  console.log('Houston neighborhood audit');
