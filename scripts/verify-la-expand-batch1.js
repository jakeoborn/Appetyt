...
const REPORT_PATH = 'scripts/verify-la-expand-batch1-report.json';
const MIN_ID = 2505;

// LA metro bbox — includes Malibu west and Calabasas / Pasadena / Monterey Park east
const BBOX = [33.70, 34.35, -118.90, -117.90];
...
async function main() {
  ...
  const declStart = html.indexOf('const LA_DATA=');
  ...
  console.log(`Verifying ${targets.length} LA expand-batch-1 entries (id >= ${MIN_ID})`);
  const report = { updated: [], confirmed: [], no_geocode: [], outside_bbox: [] };

  for (let i = 0; i < targets.length; i++) {
    const r = targets[i];
    const cleanAddr = r.address.replace(/\s*(?:Ste|Suite|Unit|Fl|#|Rooftop)\s*[\w-]+(?=[,\s])/i, '').replace(/\s+/g, ' ').trim();
