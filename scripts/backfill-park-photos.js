...
// Usage: node backfill-park-photos.js "san diego" [--apply] [--verbose]
...
const REVIEW_DIR = path.join(__dirname, '..', 'park-photo-reviews');

if (!TARGET_CITY) {
  console.error('Usage: node backfill-park-photos.js "san diego" [--apply] [--verbose]');
  process.exit(1);
}
...
async function main() {
  ...
  const dataStart = txt.indexOf('const PARK_DATA = {');
  if (dataStart < 0) { console.error('PARK_DATA not found'); process.exit(1); }
  let depth = 1, j = dataStart + 'const PARK_DATA = '.length + 1;
  ...
  if (cityIdx < 0) { console.error(`City "${TARGET_CITY}" not found in PARK_DATA`); process.exit(1); }
  let k = cityIdx;
  ...
  console.log(`\n=== ${TARGET_CITY} PARK PHOTO BACKFILL ===`);
  ...
  console.log(`\nApplied ${applyRows.length} photoUrl updates for ${TARGET_CITY} parks.`);
}
