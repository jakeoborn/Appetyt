...
const CITY = 'lv';
const VAR = 'LV_DATA';
const STATE_BOX_NV = [35.0, 42.0, -120.0, -114.0];
...
function addrCity(a) { if (!a) return null; const m = a.match(/,\s*([A-Za-z][A-Za-z\s.'-]+?),\s*NV\b/); return m ? m[1].trim() : null; }
...
for (const r of data) {
  ...
  checked++;
  if (!inBox(r.lat, r.lng, STATE_BOX_NV)) continue;
...
fs.writeFileSync(path.join(__dirname, 'lv-neighborhood-audit.json'),
  JSON.stringify({ city: 'Las Vegas', checked, in_bounds: inB, no_bounds: noB, severe: bySev.severe, moderate: bySev.moderate, edge: bySev.edge }, null, 2));
console.log('Las Vegas: checked=' + checked + ' in=' + inB + ' no-bounds=' + noB + ' severe=' + bySev.severe.length + ' moderate=' + bySev.moderate.length + ' edge=' + bySev.edge.length);
