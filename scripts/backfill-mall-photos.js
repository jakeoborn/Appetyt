...
// Usage: node backfill-mall-photos.js "san diego" [--apply] [--verbose]
...
const REVIEW_DIR = path.join(__dirname, '..', 'mall-photo-reviews');

if (!TARGET_CITY) {
  console.error('Usage: node backfill-mall-photos.js "san diego" [--apply] [--verbose]');
  process.exit(1);
}
...
function isWikipediaArticleAboutMuseum(museumName, articleTitle) {
  ...
  // Article title must contain a mall/market-y noun OR be an exact-name match for the venue.
  const museumNoun = /mall|market|shopping|center|centre|plaza|arcade|outlet|hall|district|bazaar|gallery|emporium|exchange|station|food|hub|complex|marketplace/i;
...
async function main() {
  ...
  const dataStart = txt.indexOf('const MALL_DATA = {');
  if (dataStart < 0) { console.error('MALL_DATA not found'); process.exit(1); }
  let depth = 1, j = dataStart + 'const MALL_DATA = '.length + 1;
  ...
  if (cityIdx < 0) { console.error(`City "${TARGET_CITY}" not found in MALL_DATA`); process.exit(1); }
  let k = cityIdx;
  ...
  console.log(`\n=== ${TARGET_CITY} MALL/MARKET PHOTO BACKFILL ===`);
  ...
  console.log(`\nApplied ${applyRows.length} photoUrl updates for ${TARGET_CITY} malls/markets.`);
}
