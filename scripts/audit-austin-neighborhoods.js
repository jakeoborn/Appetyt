...
function addrCity(a) { if (!a) return null; const m = a.match(/,\s*([A-Za-z][A-Za-z\s.'-]+?),\s*TX\b/); return m ? m[1].trim() : null; }
...
for (const r of data) {
  ...
  checked++;
  if (!inBox(r.lat, r.lng, STATE_BOX_TX)) continue;
...
fs.writeFileSync(path.join(__dirname, 'austin-neighborhood-audit.json'),
  JSON.stringify({ city: 'Austin', checked, in_bounds: inB, no_bounds: noB, severe: bySev.severe, moderate: bySev.moderate, edge: bySev.edge }, null, 2));
console.log('Austin: checked=' + checked + ' in=' + inB + ' severe=' + bySev.severe.length + ' moderate=' + bySev.moderate.length + ' edge=' + bySev.edge.length);
