...
const CITY = 'nyc';
const VAR = 'NYC_DATA';
const STATE_BOX_NY = [40.4, 45.1, -79.8, -71.8];
const TOL_DEG = 0.0045;

function extractDataArray(html, v) {
  const patterns = ['const ' + v + '=[', 'const ' + v + ' =[', 'const ' + v + ' = ['];
  let s = -1;
  for (const p of patterns) { const i = html.indexOf(p); if (i >= 0) { s = i + p.length - 1; break; } }
...
function addrCity(a) { if (!a) return null; const m = a.match(/,\s*([A-Za-z][A-Za-z\s.'-]+?),\s*NY\b/); return m ? m[1].trim() : null; }
...
for (const r of data) {
  ...
  checked++;
  if (!inBox(r.lat, r.lng, STATE_BOX_NY)) continue;
...
fs.writeFileSync(path.join(__dirname, 'nyc-neighborhood-audit.json'),
  JSON.stringify({ city: 'NYC', checked, in_bounds: inB, no_bounds: noB, severe: bySev.severe, moderate: bySev.moderate, edge: bySev.edge }, null, 2));
console.log('NYC: checked=' + checked + ' in=' + inB + ' no-bounds=' + noB + ' severe=' + bySev.severe.length + ' moderate=' + bySev.moderate.length + ' edge=' + bySev.edge.length);
