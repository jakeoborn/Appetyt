// Audit Chicago cards against their claimed neighborhood's bounding box.
const fs = require('fs');
const path = require('path');
const { NBH_BOUNDS, inBox, inNeighborhood } = require('./neighborhood-bounds');

const CITY = 'chicago';
const VAR = 'CHICAGO_DATA';
const STATE_BOX_IL = [36.97, 42.51, -91.52, -87.02];
...
function distFromBox(lat, lng, box) {
  const dLat = lat < box[0] ? box[0] - lat : lat > box[1] ? lat - box[1] : 0;
  const dLng = lng < box[2] ? box[2] - lng : lng > box[3] ? lng - box[3] : 0;
  return Math.sqrt(dLat * dLat + dLng * dLng);
}
function minDistToClaimed(lat, lng, n) {
  const entry = (NBH_BOUNDS[CITY] || {})[n];
  if (!entry) return 0;
...
function candidateNeighborhoods(lat, lng) {
  const table = NBH_BOUNDS[CITY]; const hits = [];
...
function extractAddrCity(addr) {
  if (!addr) return null;
  const m = addr.match(/,\s*([A-Za-z][A-Za-z\s.'-]+?),\s*IL\b/);
...
function run() {
  ...
  for (const r of data) {
    ...
    checked++;
    if (!inBox(r.lat, r.lng, STATE_BOX_IL)) { outOfState++; continue; }
  ...
  fs.writeFileSync(path.join(__dirname, 'chicago-neighborhood-audit.json'), JSON.stringify({
    city: 'Chicago', total_checked: checked, in_bounds: inBounds,
  ...
  console.log('Chicago neighborhood audit');
  console.log('  checked=' + checked + '  in=' + inBounds + '  no-bounds=' + noBounds + '  out-of-state=' + outOfState);
  ...
  for (const sev of ['severe', 'moderate', 'edge']) {
    ...
    for (const o of bySev[sev]) {
      ...
      console.log('  #' + o.id + '  ' + (o.name || '').padEnd(36).slice(0, 36) + '  [' + o.neighborhood + ']  d=' + o.dist_deg + addr + '  ' + sug);
