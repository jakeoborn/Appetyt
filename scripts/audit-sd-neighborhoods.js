...
fs.writeFileSync(path.join(__dirname, 'sd-neighborhood-audit.json'),
  JSON.stringify({ city: 'San Diego', checked, in_bounds: inB, no_bounds: noB, severe: bySev.severe, moderate: bySev.moderate, edge: bySev.edge }, null, 2));
console.log('San Diego: checked=' + checked + ' in=' + inB + ' no-bounds=' + noB + ' severe=' + bySev.severe.length + ' moderate=' + bySev.moderate.length + ' edge=' + bySev.edge.length);
