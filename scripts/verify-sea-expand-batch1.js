...
async function main() {
  ...
  const declStart = html.indexOf('const SEATTLE_DATA=');
  ...
  console.log(`Verifying ${targets.length} SEA expand-batch-1 entries (id >= ${MIN_ID})`);
  const report = { updated: [], confirmed: [], no_geocode: [], outside_bbox: [] };

  for (let i = 0; i < targets.length; i++) {
    const r = targets[i];
    const cleanAddr = r.address.replace(/\s*(?:Ste|Suite|Unit|Fl|#|Upstairs|Rooftop)\s*[\w-]+(?=[,\s])/i, '').replace(/\s+/g, ' ').trim();
